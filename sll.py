...
class LinkedList:
    ...
    def insert_back(self, value: object) -> None:
        ...
        # while node != None, get next until at last node
        while node.next:
            node = node.next
        node.next = newNode         
    ...
    def insert_at_index(self, index: int, value: object) -> None:
        ...
        if index < 0 or index > self.length():
            raise SLLException("ERROR: Invalid Index!")

        for i in range(index):
            node = node.next
        
        # set link to next node in list before inserting   
    ...
    def slice(self, start_index: int, size: int) -> "LinkedList":
        ...
        # iterate to starting index
        for i in range(start_index):
            node = node.next
        
        # copy "size" num elements to new linked list with insert_front to maintain O(n)
        for i in range( size):
            newlList.insert_front(node.value)
            node = node.next
        
        # reverse order of new linked list since added with insert_front
        curr = newlList._head.next
        while curr:
            reversedlList.insert_front(curr.value)
            curr = curr.next
